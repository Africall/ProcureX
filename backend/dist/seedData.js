...
exports.seedEnterpriseData = seedEnterpriseData;
...
async function seedEnterpriseData() {
    try {
        ...
        const enhancedItems = (data.items || []).map((item, index) => ({
            ...item,
        ...
        const enhancedSuppliers = (data.suppliers || []).map((supplier, index) => ({
            ...supplier,
        ...
        const enhancedUsers = (data.users || []).map((user, index) => ({
            ...user,
            role: ['admin', 'manager', 'technician', 'viewer'][index % 4],
        ...
        const alerts = [];
        const alertTypes = ['low_stock', 'overstock', 'expired', 'delivery_delay', 'quality_issue', 'system'];
        ...
        const systemEvents = [];
        const eventTypes = ['user_login', 'item_created', 'item_updated', 'order_placed', 'order_received', 'stock_movement', 'system_error'];
        for (let i = 1; i <= 100; i++) {
        ...
        if (db_1.dbManager.isUsingPostgres()) {
            ...
            const lowDb = await Promise.resolve().then(() => __importStar(require('./db')));
