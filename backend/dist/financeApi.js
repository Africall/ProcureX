...
router.get('/counterparties', async (req, res) => {
    ...
    `;
        const params = [];
        if (type && ['customer', 'vendor', 'staff'].includes(type)) {
            sql += ' AND type = $1';
            params.push(type);
        }
        sql += ' ORDER BY name';
...
router.post('/invoices', async (req, res) => {
    try {
        ...
        const result = await (0, financeDb_1.transaction)(async (client) => {
            // Calculate totals
            let subtotal = 0;
            let tax_total = 0;
...
router.get('/reports/kpis', async (_req, res) => {
    try {
        ...
        const revenueResult = await (0, financeDb_1.query)(`
    ...
    `);
        ...
        const expensesResult = await (0, financeDb_1.query)(`
