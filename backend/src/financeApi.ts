...
router.post('/counterparties', async (req, res) => {
  try {
    ...
    if (!displayName) return res.status(400).json({ error: 'display_name (or name) is required' });
    if (!allowedTypes.includes(ctype)) return res.status(400).json({ error: `type must be one of ${allowedTypes.join(', ')}` });
...
router.get('/payments', async (req, res) => {
  try {
    const { type, method, account_id } = req.query as any;
    let sql = `
      SELECT 
        p.id,
    ...
    const methodFilter = (method || type) as string | undefined;
    if (methodFilter) {
...
router.post('/payments', async (req, res) => {
  try {
    const {
      ...
      amount,
      account_id,
      counterparty_id, // backward-compat -> payee_id
      reference,
      memo,
      type,
      method,
