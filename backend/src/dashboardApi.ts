...
import db from './db';
import { DashboardKPI, StockMovementTrend, SupplierPerformanceMetric, PredictiveInsight, Alert } from './types';
...
router.get('/stock-trends', async (req, res) => {
  try {
    ...
    const trends: StockMovementTrend[] = [];
    for (let i = 29; i >= 0; i--) {
      const date = new Date();
...
router.get('/supplier-performance', async (req, res) => {
  try {
    ...
    const performance: SupplierPerformanceMetric[] = suppliers
