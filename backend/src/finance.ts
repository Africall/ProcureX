...
import type {
  Invoice, Bill, Payment, Expense, Account, TaxRate,
  ChartOfAccount, Transaction, TransactionLine, Customer,
  Project, CostCenter, Reconciliation, FinanceSettings
} from './types'
...
router.post('/accounts',
  body('name').isLength({ min: 1 }),
  body('type').isIn(['bank', 'mpesa', 'cash', 'card']),
...
async function updateAccountBalance(accountId: number, amount: number, isDebit: boolean) {
  const data = await dbManager.read()
...
router.post('/invoices',
  ...
  async (req, res) => {
    ...
    const newInvoiceItems = req.body.items.map((item: any, index: number) => {
...
router.patch('/invoices/:id/status',
  body('status').isIn(['draft', 'sent', 'part_paid', 'paid', 'overdue', 'cancelled']),
  async (req, res) => {
    if (!validateRequest(req, res)) return
...
router.post('/bills',
  ...
  async (req, res) => {
    ...
    const newBillItems = req.body.items.map((item: any, index: number) => {
      const lineTotal = item.quantity * item.unitPrice
      const taxRate = data.taxRates?.find(t => t.id === item.taxRateId)
      const lineTax = taxRate ? lineTotal * (taxRate.ratePercent / 100) : 0
...
router.post('/bills/from-po/:poId', async (req, res) => {
  ...
  const newBillItems = (po.items || []).map((poItem: any, index: number) => ({
    id: billItems.length + index + 1,
    billId: newId,
...
router.post('/payments',
  body('payeeType').isIn(['vendor', 'customer', 'employee', 'other']),
  body('method').isIn(['mpesa', 'bank_transfer', 'cash', 'card', 'cheque']),
...
router.post('/expenses',
  body('payee').isLength({ min: 1 }),
  body('categoryAccountId').isInt(),
  body('amount').isFloat({ gt: 0 }),
  body('date').isISO8601(),
  async (req, res) => {
    if (!validateRequest(req, res)) return
...
router.post('/tax-rates',
  body('name').isLength({ min: 1 }),
  body('ratePercent').isFloat({ min: 0, max: 100 }),
  async (req, res) => {
    if (!validateRequest(req, res)) return
...
router.get('/reports/ap-aging', async (req, res) => {
  ...
  bills.forEach(bill => {
    ...
    if (daysOverdue < 0) aging.current += bill.balance
    else if (daysOverdue <= 30) aging.days30 += bill.balance
    else if (daysOverdue <= 60) aging.days60 += bill.balance
    else if (daysOverdue <= 90) aging.days90 += bill.balance
    else aging.days90Plus += bill.balance
  })
